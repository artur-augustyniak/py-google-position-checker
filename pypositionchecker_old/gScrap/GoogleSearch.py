...
class GoogleSearch(object):
    ...
    def __init__(self,randomizer, query, lang="en", tld="com", re_search_strings=None):
        self.query = query
        self.debug = False
        #randomizer = Randomizer()
        self.browser = Browser(randomizer.getRandomProxy())
    ...
    def _extract_result(self, result):
        title, url, gurl = self._extract_title_url(result)
        desc = self._extract_description(result)
        # if not title or not url or not desc:
            # print "sdfsdfs"
            # return None
        return SearchResult(title, url, desc, gurl)
    ...
    def _extract_title_url(self, result):
        ...
        gurl = url = title_a['href']
        match = re.match(r'/url\?q=(http[^&]+)&', url)
        if match:
            url = urllib.unquote(match.group(1))
        return title, url, gurl
